...
A_TABLE_A_RUNTIME  = 0x390E2
A_TABLE_A_COUNT    = 65
A_TABLE_B_RUNTIME  = 0x391E6
A_TABLE_B_COUNT    = 20
A_TABLE_STRIDE     = 4
...
class FirmwareAnalyzerA:
    ...
    def load(self, path: str):
        ...
        self._detect_code_boundary()
        self.decoded = bytearray(self.payload_size)
        for i in range(min(self.code_section_len, self.payload_size)):
            self.decoded[i] = self.raw[A_PAYLOAD_START + i] ^ A_XOR_KEY
        for i in range(self.code_section_len, self.payload_size):
            self.decoded[i] = self.raw[A_PAYLOAD_START + i]
    ...
    def _find_panel_init_tables(self):
        self.panel_entries.clear()
        tables = [
            ("A", A_TABLE_A_RUNTIME, A_TABLE_A_COUNT),
            ("B", A_TABLE_B_RUNTIME, A_TABLE_B_COUNT),
        ]
        for table_id, runtime_addr, entry_count in tables:
            base_poff = self.runtime_to_payload(runtime_addr)
            if base_poff < 0 or base_poff + entry_count * A_TABLE_STRIDE > self.payload_size:
                continue
            for grp in range(entry_count):
                grp_off = base_poff + grp * A_TABLE_STRIDE
                for pair in range(2):
                    poff = grp_off + pair * 2
                    if poff + 1 >= self.payload_size:
                        break
                    reg = self.decoded[poff]
                    val = self.decoded[poff + 1]
                    foff = self.payload_to_file(poff)
                    self.panel_entries.append(PanelInitEntry(
                        file_offset=foff, payload_offset=poff, reg=reg,
                        value=val, original=val, table_id=table_id,
                        group_idx=grp, pair_pos=pair))
    ...
    def set_panel_value(self, entry: PanelInitEntry, new_val: int):
        ...
        if val_poff < self.code_section_len:
            self.raw[foff] = new_val ^ A_XOR_KEY
        else:
            self.raw[foff] = new_val
    ...
    def set_decoded_byte(self, payload_offset: int, new_val: int, desc: str = ""):
        ...
        if payload_offset < self.code_section_len:
            self.raw[foff] = new_val ^ A_XOR_KEY
        else:
            self.raw[foff] = new_val
        self.mods.append(ModA(foff, old, new_val,
...
class App:
    ...
    def _build_b_overview(self):
        ...
        ttk.Button(bf, text="Open SPI Flash…",
                   command=self._open_b_flash).pack(side="left")
    ...
    def _refresh_a_overview(self):
        ...
        lines = [
            ...
            f"  Encoding:       Hybrid XOR 0x55  "
            f"(code: 0x{fw.code_section_len:X} bytes,"
            f"  data: {fw.payload_size - fw.code_section_len:,} bytes)",
    ...
    def _build_a_hex(self):
        ...
        jumps = [
            ("Vectors", "0"),
            ("Table A", f"0x{A_TABLE_A_RUNTIME - A_FLASH_BASE:X}"),
            ("Table B", f"0x{A_TABLE_B_RUNTIME - A_FLASH_BASE:X}"),
            ("Data Sec", f"0x{0x35600:X}"),
        ]
...
def main():
    ...
    if not b_path:
        for c in ["tw8836_mcu.bin",
                   "firmware/fix/SKY04X_Pro_B_APP_V4.0.2.bin"]:
            if os.path.isfile(c):
                b_path = c
                break
    if not a_path:
        for c in ["firmware/sky04xpro_v4.1.4/SKY04XPro_A_APP_V4.1.4.bin",
